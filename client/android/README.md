...
2. **智能重连**: 单轨指数退避重连 (3s → 6s → … → 60s)，避免与 Paho 自动重连双轨打架
3. **持久会话**: `cleanSession=false`，配合重连后 HTTP 历史补洞
4. **MQTT keepAlive**: 默认 120 秒（连接时最低按 60 秒生效），减轻 Doze 下被服务端踢线
5. **保活闹钟**: 精确闹钟 + 前台服务；小米等 ROM 仍建议关闭电池优化、允许自启动
